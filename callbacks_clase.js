...
function getGame(id, callback) {
    // Codigo
    setTimeout(()=>{callback(null,games.filter((game) => game.id === id)[0])}, 1000);
}
...
function getTipo(id, callback) {
    // Código 
    setTimeout(()=>{callback(null,tipo[games.filter((game) => game.id === id)[0].tipo_id])}, 1000);
}
...
getGames((err, games) => {
    ...
    console.log("El tipo de "+games[1].nombre)
